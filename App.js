	...
	search = text => {
		if (text) {
			this.setState({ text, list: [], loading: true }, () => {
				let url = `https://recruitment.theasia.com/Activities/search_proto?input=${
					this.state.text
				}`;
				fetch(url, {
					method: "GET",
	...
	itemLoader = item => {
		...
		return (
			...
			>
				...
				<View style={{ flex: 1 }}>
					<Text style={[styles.textColorGray, styles.contentText]}>
	...
	emptyList = () => {
		...
		if (text && !loading) {
			return (
				<View style={styles.emptyList}>
					<Text style={styles.textColorGray}>
						Sorry, There is no result related to your criteria. Please change it
						and retry.
					</Text>
				</View>
			);
			...
			return (
				<View style={styles.emptyList}>
					<Text style={styles.textColorGray}>
						Please type something to view the result
					</Text>
				</View>
			);
		}
